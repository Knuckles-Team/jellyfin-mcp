...
> **Install the connector-focused `[mcp]` extra.** Examples use `jellyfin-mcp[mcp]` to add
...
`jellyfin-mcp` can run as a local stdio process or container, or behind a remote
network boundary. The
[Deployment guide](https://knuckles-team.github.io/jellyfin-mcp/deployment/) carries
...
services:
  jellyfin-mcp-mcp:
    image: example/jellyfin-mcp:mcp
...
|-------|----------|----------|
| `jellyfin-mcp[mcp]` | Connector-focused MCP server (`agent-utilities[mcp]` — FastMCP/FastAPI + `epistemic-graph[full]`) | You only run the **MCP server** (smallest install / image) |
| `jellyfin-mcp[agent]` | Agent runtime (`agent-utilities[agent-runtime,logfire]` — model orchestration + `epistemic-graph[full]`) | You run the **integrated agent** |
...
|-----------|--------------|----------|------------|
| `example/jellyfin-mcp:mcp` | `--target mcp` | `jellyfin-mcp[mcp]` — **connector-focused**, includes `epistemic-graph[full]`; no model-orchestration stack | `jellyfin-mcp` |
| `example/jellyfin-mcp@sha256:<digest>` | `--target agent` (default) | `jellyfin-mcp[agent]` — **agent runtime**, model orchestration + `epistemic-graph[full]` | `jellyfin-agent` |

```bash
docker build --target mcp   -t example/jellyfin-mcp:mcp    docker/   # connector-focused MCP server
docker build --target agent -t example/jellyfin-mcp:agent-local docker/   # agent runtime
```
...
| `JELLYFIN_API_KEY` | secret-injected | Authentication values (choose API key or Username/Password) API key standard input: |
| `JELLYFIN_USERNAME` | `admin` | Credential-based login (used if API key is not provided): |
| `JELLYFIN_PASSWORD` | secret-injected |  |
...
| `DELEGATED_SCOPES` | `api` |  |
| `JELLYFIN_AUDIENCE` | `https://jellyfin.example.com` |  |
| `OIDC_TOKEN_ENDPOINT` | secret-injected |  |
...
| `JELLYFIN_TLS_PROFILE` | Named TLS profile for private PKI, mTLS, or proxy policy | — |
| `JELLYFIN_TLS_PROFILE_REF` | Secret reference containing the TLS profile | — |
...
to **"deploy `jellyfin-mcp` with agent-utilities-deployment"**.

| Install mode | Command |
|------|---------|
| Installed package | `uv tool install "jellyfin-mcp[mcp]"`, then run `jellyfin-mcp` |
| Editable source | `uv pip install -e ".[agent]"`, then run `jellyfin-mcp` |
| Immutable container | deploy `registry.example.invalid/jellyfin-mcp@sha256:<digest>` through the operator-selected orchestrator |
