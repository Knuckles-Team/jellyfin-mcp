# Concept Registry — jellyfin-mcp

> **Prefix**: `CONCEPT:JELLYFIN-*`
> **Version**: 0.15.0
...
|------------|------|-------------|
| `CONCEPT:JF-OS.governance.jellyfin` | Condensed Jellyfin Operations | MCP tool domain `condensed_jellyfin` — Action-routed dynamic tool registration |
...
This project integrates with `agent-utilities` via `CONCEPT:AU-ECO.messaging.native-backend-abstraction` (Unified Toolkit Ingestion). The `jellyfin_mcp` MCP server registers its tools with the agent-utilities FastMCP middleware, enabling automatic discovery, telemetry, and Knowledge Graph ingestion of all JELLYFIN-* concepts.
